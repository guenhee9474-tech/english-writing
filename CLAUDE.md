...
`dictTranslate(q)` 가 순서대로 시도한다.

1. **국립국어원 한국어기초사전(krdict)** — 한국어→영어만. 진짜 사전이라 표제어·품사·뜻풀이를 준다.
   원래 형태로 찾고, 없으면 `koBaseForm(q)`(배고픈→배고프다)로 다시 찾는다.
   인증키는 **코드에 적지 않는다**(공개 저장소). 제출명단 파일의 **[설정] 탭 B2 칸**에 넣는다(`settingsSheet()` 가 탭을 만든다). 스크립트 속성 `krdictKey` 도 예비로 읽는다.
   키는 https://krdict.korean.go.kr/kor/openApi/openApiRegister 에서 이메일로 무료 발급(하루 5만 건).
   `UrlFetchApp` 을 쓰므로 **처음 한 번 "외부 서비스 연결" 승인**이 필요하다. 사용자는 편집기에서 **`사전켜기`** 하나만 실행하면 된다(승인 → 키 확인 → 시험까지 그 함수가 안내한다). 사용자에게 코드를 고치게 하지 말 것.
   krdict 호출은 `num` 이 10 이상이어야 하고 `User-Agent` 헤더가 있어야 한다(없으면 각각 103 오류·400 차단). 사전이 오류를 돌려주면 [오류] 탭에 남는다.
2. **구글 번역 + 되돌림 확인** (`dictByTranslate`) — 사전에 없거나 krdict 가 실패하면. 영어→한국어는 항상 이쪽.
   번역 결과를 다시 되돌려 `sameWord()` 로 원래 단어와 맞는지 본다. 안 맞으면 기본형으로 재시도.
3. 그래도 확인이 안 되면 학생 화면에 "뜻이 확실하지 않습니다" 경고를 붙인다.

**krdict 가 실패하면(키 없음·권한 미승인·응답 실패) 조용히 2번으로 넘어간다.** 그래서 켜지 않아도 지금과 똑같이 동작한다.
`trCached()`·krdict 결과 모두 6시간 보관해 같은 단어 반복 조회 비용을 없앤다.
**신뢰도를 직접 보려면 편집기에서 `testDict()` 를 실행**하고 실행 로그의 표(단어·결과·출처·확인·기본형·품사·뜻풀이)를 본다.
