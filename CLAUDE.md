...
| 사전 | 단어 사전 조회 기록 |
| 총괄 / 반별 요약 | `rebuildOverview()`가 다시 쓴다. **제출 때 자동 갱신하지 않는다**(30명이 몰릴 때 실행 시간 제한을 넘겼다). 수업 전에 편집기에서 `installOverviewTrigger()`를 한 번 실행하면 5분마다 갱신되고, 끝나면 `removeOverviewTrigger()`. 즉시 보려면 `rebuildOverview()`를 직접 실행. "명단" 탭(학번, 이름)이 있으면 미접속 학생도 표시 |
...
2. 편집기에서 `installOverviewTrigger()` 한 번 실행(총괄 시트 5분마다 갱신). 수업이 끝나면 `removeOverviewTrigger()`.
3. 아이패드 한 대로 로그인해 화면 아래 버전 글자가 `CONFIG.version`과 같은지 확인.
4. 수업 중 문제가 보이면 제출명단 파일의 **오류** 탭을 먼저 본다. 이제 잠금 실패·PDF 실패·시트 기록 실패·이름 불일치·JSON 초과가 모두 여기 남는다.

## 보류된 과제 (사용자와 상의 후 진행)

- 보안: 페이지 소스에 교사 PIN과 token이 보인다. 교사 기능을 서버 검증으로 옮기고 요청별 총량 제한을 두는 작업이 남아 있다. 현재 PIN(9474)은 사용자의 GitHub 계정 이름에 든 숫자와 같아 추측 가능하다.
